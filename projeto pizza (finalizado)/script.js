...
c('.pizzaInfo--addButton').addEventListener('click', ()=>{
 let size =parseInt(c('.pizzaInfo--size.selected').getAttribute('data-key'));
  ...
  let key = carr.findIndex((item)=>item.identifier == identifier);
...
function updateCarr(){
    ...
    c('.menu-openner span').innerHTML = carr.length;
    if(carr.length >0) {
        ...
        for(let i in carr){
            ...
            subtotal += pizzaItem.price * carr[i].qt;

            let carrItem = c('.models .cart--item').cloneNode(true);
            ...
            carrItem.querySelector('.cart--item--qt').innerHTML = carr[i].qt;
